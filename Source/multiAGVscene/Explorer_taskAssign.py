...
class Explorer:
    def __init__(self, layout, veh_name="veh1", icon_name="veh1", start_position = [1, 1]):
        self.layout = layout
        """--------------init attributes--------------"""
        """work time """
        ...
        """"init"""
        self.init(start_position)
        self.conflict = False
        self.working = 0

    def init(self, start_position):
        self.last_place = [1, 1]
        self.current_place = start_position
        self.task_order = 0
        self.task_stage = 0
        self.running_state = "Start"
        self.loaded = False
    ...
    def get_task(self):
        ...
        if len(self.layout.task_list) == len(self.layout.task_arrangement[0]) and self.task_stage == 0:
            ...
            if len(self.layout.task_list) == sum(self.layout.task_arrangement[2]) + 1:
                self.layout.task_finished = True
                self.running_state = "AllTaskFinished"
            return
        """"assign task"""
        
        for i in range (0, len(self.layout.task_list)):
            if self.layout.task_list[i][4] == self.explorer_name:
                self.task_order = i
            break
        ...
        if self.task_stage == 0:
            ...
            # print("task order: ", self.layout.task_arrangement)
            # print("sum: ", sum(self.layout.task_arrangement[2]))
            # print("task list: ", self.layout.task_list) 
            # print("explorer_name", self.explorer_name)
    ...
    def continue_working(self):
        ...
        if self.task_stage == 3:
            ...
            index = self.layout.task_arrangement[0].index(self.task_order)
            print(index)
            self.layout.task_arrangement[2][index] = 1
            # Xoa nhiem vu da lam 
            task = list(self.layout.task_list[self.task_order])
            task[4] = "done"
            self.layout.task_list[self.task_order] = tuple(task)
...
#         if len(action_list) <= self.checklen or self.checklen <= 1:
#             self.checklen = len(action_list)
#         if len(action_list) < self.checklen:
#             self.optimal_path = 1
            
#         if find_target == False or len(action_list) > self.checklen:
#             action_str = "STOP"
#         else:
