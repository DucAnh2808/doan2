...
class Scene:
    ...
    def init(self):
        ...
        for explorer in self.explorer_group:
            explorer.init(start_position = explorer.current_place)
        """observations"""
    ...
    def run_mode(self, running_type="manual", smart_controller=None):
        ...
        for index, explorer in enumerate(self.explorer_group):
            x = random.randrange(1, 21, 2)
            y = random.randrange(1, 21, 2)
            new_place = [x, y]
            explorer.current_place = new_place

        while True:
            # pygame.time.delay(300)      #Delay moi buoc di cua AGV
            self.running_time += 1
            if self.render:
            ...
            for index, explorer in enumerate(self.explorer_group):
                ...
                if self.layout.task_finished:
                    ...
                    print("end_run", datetime.datetime.now() - self.start_time)
                    print("Cost distance: ", self.total_distance)
                    print("Total_optimal_path: ", self.total_optimal)
                ...
                if running_type in ["A_star", "auto"]:
                    ...
                    if explorer.optimal_path == 1:
                        self.total_optimal += 1
                        explorer.optimal_path = 0
