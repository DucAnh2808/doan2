...
class MADQNAgentController:
    ...
    def create_agent(self, map_xdim, map_ydim):
        ...
        if self.control_mode == "train_NN":
            ...
            print("load NN")
            policy_net = torch.load(os.path.join(self.storage_path, "1rb_w/1rb_w_p.pt"))
            target_net = torch.load(os.path.join(self.storage_path, "1rb_w/1rb_w_t.pt"))
        '''create Agent object'''
    ...
    def save_neural_network(self, auto=False):
        ...
        if auto:
            ...
            torch.save(self.agent.policy_network, os.path.join(self.storage_path, "1rb_w_p_a.pt"))
            torch.save(self.agent.target_network, os.path.join(self.storage_path, "1rb_w_t_a.pt"))
        else:
            print("final neural network saved")
            torch.save(self.agent.policy_network, os.path.join(self.storage_path, "1rb_w_p.pt"))
            torch.save(self.agent.target_network, os.path.join(self.storage_path, "1rb_w_t.pt"))
